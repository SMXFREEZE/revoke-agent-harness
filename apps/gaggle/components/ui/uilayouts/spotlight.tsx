// @ts-nocheck
'use client';
import { cn } from "@/lib/utils/cn-tw";
...
export function SpotLightItem({ children, className }: SpotlightItemProps) {
  ...
  return (
    <div
      ...
      ref={boxWrapper}
      className={cn(className, ' relative rounded-[inherit] p-px bg-[color:var(--accent-tint)] overflow-hidden')}
    >
      {isHovered && (
        <div
          className='pointer-events-none absolute opacity-0 z-50 rounded-[inherit] w-full h-full group-hover:opacity-100  transition duration-300 '
          style={{
            mixBlendMode: 'screen',
            background: `radial-gradient(220px circle at ${overlayColor.x}px ${overlayColor.y}px, rgb(var(--accent-glow) / 0.55), transparent 75%)`,
          }}
        />
      )}
      {HoverFocusSpotlight && (
